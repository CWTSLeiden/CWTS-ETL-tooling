...
header = textscan(fid, '%s %s %s %s %s %s %s %s %s %s %s %s %s %s', 1, 'Delimiter', '\t');
data = textscan(fid, '%d %d %d %d %s %s %s %s %s %s %s %s %s %s', 'HeaderLines', 1, 'Delimiter', '\t');
fclose(fid);

labels = cat(2, data{5:end});
header = cat(2, header{:});
...
for row=1:n_rows
    fprintf(fid, '%d\t%d\t%d\t%d\t', output_data(row,:));
